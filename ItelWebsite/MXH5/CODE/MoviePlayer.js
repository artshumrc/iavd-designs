...
function H5MoviePlayer(p){this.player=p;}
H5MoviePlayer.prototype=new MoviePlayer();
/*************************************************/

function H5MoviePlayer(p){
  this.player=p;
  this.fatalError=null;
  this.stopFrame=null;
  this.intervalID=null;
  this.index=H5MoviePlayer.instances.length;
  this.selfRef="H5MoviePlayer.instances["+this.index+"]";
  H5MoviePlayer.instances[this.index]=this;
  // this.setConfig();
}
H5MoviePlayer.prototype=new MoviePlayer();
H5MoviePlayer.instances=new Array();

H5MoviePlayer.prototype.setConfig=function(){
  if(this.player && this.player.setConfig)
    return this.player.setConfig(fpConf(config));
  setTimeout(this.selfRef+".setConfig()",100);
}
function h5_getMovieTime(){return this.player.currentTime;}
function h5_setMovieTime(t){this.player.currentTime=t;}
/*
function h5_setMovieTime(t){try{this.player.currentTime=t;
               }catch(e){alert("setMovieTime("+t+"):"+(e.description?ob2String(e):e));} };
*/
function h5_getMovieTimeScale(){return 1;}
function h5_setMovieTimeScale(s){}
function h5_getMaxTime(){return this.player.duration;}
function h5_isPlaying(){return !this.player.paused && !this.player.ended && this.player.readyState >= this.player.HAVE_CURRENT_DATA;}
function h5_pause(){return this.player.pause();}
function h5_isPaused(){return this.player.paused;}
// function h5_playRange(a,b){return this.player.playRange(a,b);}
function h5_clearRange(){}
function h5_getMovieState(){
 if(testing)alert("readyState="+this.player.readyState); 
  return "";}
/**
PercentLoaded=function PercentLoaded() {
FrameLoaded=function FrameLoaded()
**/
function h5_getMovieRate(){return this.player.playbackRate;}
function h5_setMovieRate(r){this.player.playbackRate=r}
function h5_setColor(r){}
function h5_getVersion(){return "H5 MannX 1.1";}
function h5_hasHadFatalError(){return null!=this.fatalError;}
function h5_getFatalError(){return this.fatalError;}
function h5_isLoaded(){return this.player.readyState == this.player.HAVE_ENOUGH_DATA;}
function h5_play(){this.player.play();}
function h5_stop(){ if(this.isPlaying() ) this.player.pause(); }
function h5_killInterval(){
  this.stop();if(!this.intervalID)return;
  clearInterval(this.intervalID);this.intervalID=null;
}

function h5_playRange(start,len){
try{
  this.killInterval();
// alert("playRange(start="+start+",len="+len+")");
//  this.player.pause();
  if(!this.player.readyState>=this.player.HAVE_CURRENT_DATA)
    return "cannot play "+sec2MinSec(start)+" until more data is loaded."
...
function h5_checkStopTime(){
  if(!this.stopTime)return this.killInterval();
  if(this.stopTime > this.getMovieTime())return;
  this.killInterval();
}

function h5_handleError(){ // from http://www.w3.org/TR/html5/the-video-element.html
   // video playback failed - show a message saying why
   // <p><video src="tgif.vid" autoplay controls onerror="h5_handleError(event)"></video></p>
...
h5Fns={
pause:h5_pause,
isPaused:h5_isPaused,
getMovieTime:h5_getMovieTime,
setMovieTime:h5_setMovieTime,
getMovieTimeScale:h5_getMovieTimeScale,
setMovieTimeScale:h5_setMovieTimeScale,
getMaxTime:h5_getMaxTime,
isPlaying:h5_isPlaying,
playRange:h5_playRange,
clearRange:h5_clearRange,
getMovieState:h5_getMovieState,
getMovieRate:h5_getMovieRate,
setMovieRate:h5_setMovieRate,
setColor:h5_setColor,
getVersion:h5_getVersion,
hasHadFatalError:h5_hasHadFatalError,
getFatalError:h5_getFatalError,
isLoaded:h5_isLoaded,
play:h5_play,
stop:h5_stop,
killInterval:h5_killInterval,
checkStopTime:h5_checkStopTime
   };
 for(var f in h5Fns)H5MoviePlayer.prototype[f]=h5Fns[f];
}
...
obS+='</video>\n';
// alert("video element constructed as \n"+obS);
return obS;
}

//    <video id="video" width="320" height="240" poster="algo0101.jpeg" controls="controls" preload="auto" autobuffer>
//      <source src="algo0101.mp4" type="video/mp4"></source>
//      <source src="algo0101.ogg" type="video/ogg"></source>
//      <param name="src" value="algo0101.mp4" />
...
function generateMoviePlayerElement(config){
  ...
  if(config.movie.playerClass=="H5MoviePlayer"){
    S+=generateH5VideoElement(config);
  } else if(config.movie.playerClass=="FlashSWFMoviePlayer"){  // set within config.js
