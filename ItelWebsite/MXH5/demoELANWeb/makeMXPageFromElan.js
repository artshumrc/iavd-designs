...
function tMhandleDoc(){return tierManager.handleDoc();}
...
// alert("found TIER ["+i+"]=["+tierAttrOb.TIER_ID+"]");
  ...
  timeSlots:{},
  init:function(iframeID,formName){ // assumes iframe is loaded with form;
    this.form=document.forms[formName];
    this.iframe=$(iframeID);
    this.srcFileName=this.form.unitName.value;
    try{ this.doc=this.iframe.contentDocument.getElementsByTagName("ANNOTATION_DOCUMENT")[0].parentNode ;}
    catch(e){return alert("failed to find 'ANNOTATION_DOCUMENT' element:"+e);};
    this.handleDoc();
  },
  maxReadTries:10,
  getDoc:function(N){
  ...
  alert("readElanTiers called");
    var A=this.doc.getElementsByTagName("TIER");
  alert("there are "+A.length+" tiers");
    this.topTiers=[];
    this.tierIDs=[];
    this.tierAttributes={};
...
// if(!confirm("value of ra["+refID+"] is ["+val+"]"))throw "up";
  ...
  handleDoc:function(){ // assumes this.doc is the ANNOTATION_DOCUMENT element's parentNode of an EAF
    try{
    var B=[];
    alert("doc=\n\n"+this.doc);
    this.readElanTagAttrs();
    alert("x0");
    this.readElanTagAttrsSeqs();
    alert("x1");
    this.tierVals=this.tierAttrsSubAttrsSubSubVals();
    alert("x2");
    this.doc=this.doc.getElementsByTagName("ANNOTATION_DOCUMENT")[0];
    this.topAttrs=this.getAttrObject(this.doc);
    alert("x3");
    this.readElanHeader();
    alert("x4");
    ...
    }catch(e){return alert("handleDoc: "+e);}
  },
...
S+= '<meta http-equiv="Content-Type" content="text/html;charset=utf-8" >\n';
    ...
    // boxes for dict,comm,script search choice hidden along with configData (and initial everything)
    S+= " <style>\n  #configData {display:none;}\n #boxes {display:none;}\n  .uninitialized {display:none;} </style> \n";
    S+= '<script src="'+root+'CODE/mfDomLib.js"></script> \n';
    S+= '<script src="'+root+'MX.js"></script> \n';
  S+= '</head> \n';
  S+= '<body onload="mxLoad()" class="uninitialized"> \n';
...
function onChangeUnit(frm){
  ...
  frm.fileName.value=F[F.length-1]+".webm"; // after last "/"
}
